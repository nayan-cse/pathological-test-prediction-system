...
export async function GET(req) {
  ...
  try {
    ...
    logger.info(`Fetching reviewed reports for Doctor ID: ${id} (Page ${page}, Limit ${limit})`);
    ...
    if (!results || results.length === 0) {
      logger.info(`No reviewed reports found for Doctor ID: ${id} on page ${page}`);
      
      if (page === 1) {
        return NextResponse.json({ 
          message: 'No reviewed reports found.', 
          data: [],
          pagination: {
            total: 0,
            totalPages: 0,
        ...
        // If user requested a page that doesn't exist but there are results on other pages
        return NextResponse.json({ 
    ...
    logger.info(`Successfully retrieved ${results.length} reviewed report(s) for Doctor ID: ${id} (Page ${page}/${totalPages})`);
