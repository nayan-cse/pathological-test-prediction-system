...
export async function POST(req) {
    const { name, email, phone_number, password, role, date_of_birth } = await req.json();

    if (!name || !email || !phone_number || !password || !date_of_birth) {
        logger.error('Registration failed: Missing required fields');
        return NextResponse.json({ error: 'All fields are required.' }, { status: 400 });
    }
    ...
    try {
        ...
        const hashedPassword = await bcrypt.hash(password, 10);
        const result = await query('INSERT INTO users (name,email,phone_number, password, role, date_of_birth) VALUES (?, ?, ?, ?, ?, ?)', [
            name,
            email,
            phone_number,
            hashedPassword,
